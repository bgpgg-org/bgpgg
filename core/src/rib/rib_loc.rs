...
impl LocRib {
    ...
    pub fn mark_peer_routes_stale(
        ...
        let prefixes = match (afi_safi.afi, afi_safi.safi) {
    ...
    pub fn remove_stale_routes_from_peer(
        &mut self,
        peer_ip: IpAddr,
        afi_safi: crate::bgp::multiprotocol::AfiSafi,
    ) -> Vec<IpNetwork> {
    ...
    /// Clear all stale route tracking for a peer (all AFI/SAFIs)
    /// Used when GR is cancelled or peer is shut down
    pub fn clear_all_stale_for_peer(&mut self, peer_ip: IpAddr) {
        self.stale_routes.retain(|(ip, _), _| *ip != peer_ip);
    }
}
