...
impl Ord for Path {
    ...
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        // 1. Prefer path with highest LOCAL_PREF
        ...
        // RFC 4271 Section 9.1.2.2 Tie-Breaking (Phase 2)
        // From here on, we only compare routes learned from peers

        // a) Prefer path with shortest AS_PATH
        ...
        // b) Prefer path with lowest ORIGIN (IGP < EGP < INCOMPLETE)
        ...
        // c) Prefer path with lowest MED
        // Only compare if both have MED set (or treat missing as 0)
        // TODO: Should only compare MED from same neighboring AS
        if let (Some(self_med), Some(other_med)) = (self.med, other.med) {
        ...
        // e) Lowest interior cost to NEXT_HOP - skipped (not implemented)

        // f) Lowest BGP Identifier - using peer address as proxy
        // g) Lowest peer address (tie-breaker for determinism)
        match (&self.source, &other.source) {
