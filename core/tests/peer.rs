...
async fn test_mrai_rate_limiting() {
    ...
    for (mrai_secs, num_routes) in test_cases {
        ...
        // Update server1's MRAI setting if needed (note: this test may need adjustment
        // if the setting cannot be changed after peering is established)
        _ = mrai_secs; // The test may need redesign to apply MRAI during add_peer
